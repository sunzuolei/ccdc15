...
wp_sim=600; hp_sim = 500; hp_img = 268;% hp_img is the height of image and the unit is pixel.
l_sim = 0.01; 
...
%% update the eagle eye.
       ...
       if ~isempty(cov)
        ...
        if ii~=1 && ~isempty(landmark(ii-1).z)&&~isempty(lmFastSLAM(ii-1).x)
           ...
           ax_eye= [xFastSLAM(1,ii)- 0.01*h.eyelim.x, xFastSLAM(1,ii)+0.01* h.eyelim.x,...
                   xFastSLAM(2,ii)-0.01*h.eyelim.y,xFastSLAM(2,ii)+ 0.01*h.eyelim.y]; 
